...
1. 在『数据预处理』工作区以“自定义使用”入口导入自己的评论数据（Excel/CSV 或飞书多维表格），由本地模型在线标注（每批 10–20 条，语料不少于 10 条）后发布；至少发布三个语料包（基线 / 挑战 / holdout，可选更多作为增量批）。自定义语料全部进入 ANALYSIS 允许集合，不套用官方案例的数据规模与配额。
2. 进入『自由决策实验』工作区（看完实时决策看板回放后解锁），选择各语料包并启动基线分析。
3. 按阶段推进决策链：应用挑战批 → 确认选线 → holdout 验证 → 冻结快照 → 逐批释放增量，页面实时展示排名、压力检查、文本修订与换线建议。

官方案例的冻结产物由同一套链路离线生成（`tools/run_official_decision.py`）；自由实验的结果仅保留在当前会话中用于核对，不会写回冻结数据。线上公开部署不配置模型密钥，因此该模块仅本地可用。
