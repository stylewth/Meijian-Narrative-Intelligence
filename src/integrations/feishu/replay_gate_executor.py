...
from .result_projection import add_demo_context, run_log_fields
...
def replay_gate_elements(
    ...
    *,
    ...
    传入当前进度游标时，未解锁到对应里程碑的门不显示，避免提前展示后续内容。
    """
    ...
    if locked:
        elements.append(
            _text_element(
                ...
                "尚未解锁：" + "、".join(locked) + "；请先推进网页进度或用「下一步」远端推进。",
            )
        )
    return elements
