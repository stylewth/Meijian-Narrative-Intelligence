...
.mj-stage-track{position:relative;margin:.8rem 0 1.15rem;padding:.1rem 0}.mj-stage-track ol{position:relative;z-index:1;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0;margin:0;padding:0;list-style:none}.mj-stage-rail{position:absolute;z-index:0;left:10%;right:10%;top:.58rem;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));align-items:center}.mj-stage-connector{position:relative;height:5px;margin:0 .4rem;overflow:hidden;border-radius:99px;background:#DDD4CB}.mj-stage-connector::before{content:"";position:absolute;inset:0;background:linear-gradient(90deg,#C9A86A 0 72%,#F0DDB0 86%,#C9A86A 100%);background-size:9rem 100%;transform:scaleX(0);transform-origin:left center}.mj-stage-connector.is-complete::before{transform:scaleX(1);animation:mjStageTrackFill .78s var(--mj-ease-curtain) both}.mj-stage-connector.is-flowing::before{transform:scaleX(1);animation:mjStageTrackFlow .82s linear both}.mj-stage-node{position:relative;display:grid;grid-template-columns:1fr;grid-template-rows:1.2rem auto auto;justify-items:center;row-gap:.12rem;align-items:center;min-width:0;color:#988D85}.mj-stage-dot{position:relative;z-index:2;box-sizing:border-box;grid-row:1;grid-column:1;display:grid;place-items:center;width:.98rem;height:.98rem;border:1px solid #CFC2B6;border-radius:50%;background:#F6F5F2;box-shadow:0 0 0 .22rem #F6F5F2;color:#8C8178;font-family:Georgia,serif;font-size:.56rem}.mj-stage-node strong{grid-row:2;grid-column:1;margin-top:.15rem;overflow:hidden;max-width:100%;color:#5B514B;font-family:STZhongsong,"华文中宋",serif;font-size:.92rem;text-align:center;text-overflow:ellipsis;white-space:nowrap}.mj-stage-node small{grid-row:3;grid-column:1;font-size:.68rem;text-align:center}.mj-stage-node.is-complete .mj-stage-dot{border-color:#C9A86A;background:#C9A86A;color:#FFF8EF}.mj-stage-node.is-current .mj-stage-dot{border:1px solid var(--mj-polish-wine);box-shadow:0 0 0 .22rem #F6F5F2,0 0 0 .32rem rgba(143,47,77,.1);color:var(--mj-polish-wine)}.mj-stage-node.is-current strong{color:var(--mj-polish-wine)}.mj-stage-node.is-blocked{opacity:.52}.mj-stage-seal{position:absolute;right:.15rem;top:-.75rem;padding:.08rem .28rem;border:1px solid rgba(143,47,77,.5);color:var(--mj-polish-wine);font-family:STZhongsong,"华文中宋",serif;font-size:.56rem;letter-spacing:.08em;transform:rotate(-5deg)}
